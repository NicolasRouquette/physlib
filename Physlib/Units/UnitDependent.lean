...
@[simp]
lemma UnitChoices.dimScale_scaleUnit {u1 u2 u : UnitChoices} (d : Dimension PhyslibBase) :
...
def DimSet (M : Type) [MulAction ℝ≥0 M] [MulUnitDependent M] (d : Dimension PhyslibBase) :
    Set M :=
  {m : M | ∀ u1 u2, scaleUnit u1 u2 m = (UnitChoices.dimScale u1 u2 d) • m}

instance (M : Type) [MulAction ℝ≥0 M] [MulUnitDependent M] (d : Dimension PhyslibBase) :
